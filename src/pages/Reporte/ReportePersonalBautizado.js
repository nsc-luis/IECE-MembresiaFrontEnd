...
export default function ReportePersonalBautizado(){
    ...
    console.log(dto)
    console.log(JSON.parse(localStorage.getItem("infoSesion")))
    //Llamadas en render
    useEffect(() => {
        if(sector == null){
    ...
    const reportePersonalBautizadoPDF = () =>{
        ...
        personas.map((persona, index) => {
            if(persona.persona.per_Categoria === "ADULTO_HOMBRE"){
                doc.text(`${index + 1}.- ${persona.persona.per_Nombre} ${persona.persona.per_Apellido_Paterno} ${persona.persona.per_Apellido_Materno}`, 20, yAxis);
                yAxis+=5;
            }
        })
